...
- [x] list with filters, get, cancel — new `internal/cp/jobs` package over the control-plane `jobs` table (the history/mirror view, C-3, ADR-0005; never a tenant DB, ADR-0003), tenant from the API key (FR-ACC-03). `GET /v1/jobs` (`?status&kind&source&limit&cursor` → `{items,next_cursor}`, filters validated against the enums), `GET /v1/jobs/{id}` (status, `attempt`, `stats`, timing, computed `duration_ms`), `POST /v1/jobs/{id}/cancel` per SPEC-08 §4: a **queued** job is cancelled now (guarded `where status='queued'` mirror-row flip, 200 — fully effective, no worker holds it); a **running** job's cooperative cancel is the EPIC-09 River `Canceller` seam (nil today → `not_found` seam; wired → 202 + the worker writes `running`→`cancelled`, SPEC-08 §3); terminal → 409; already-cancelled → idempotent 200. Flipping a running row in the API was rejected as a fake. Wired into `internal/api` (`New` + `liveRoutes()`) behind `RequireScopeAdmin → RateLimit`; `api/openapi.yaml` regenerated (drift guard green). No migration/schema change (table/enums/index already existed). TDD; unit tests (`internal/cp/jobs`) + e2e golden path (`test/e2e/jobs_e2e_test.go`, incl. FR-ACC-03 cross-tenant isolation) over the real control-plane Postgres; `TestTenantIsolationSuite` re-run (SPEC-01 §9). ADR-0031; ISSUE-0004. Audit of the cancel action (FR-ADM-05) rides EPIC-09 per the STORY-03.6 plan.

### STORY-04.6 — Admin tenant endpoints (FR-TEN-01/05/07)
- [x] create, list, patch (status/connection/settings), delete with grace — thin HTTP layer in `internal/cp/tenants` (`AdminService`/`AdminHandlers`/`AdminPoolStore`) over the **existing** backend (do not rebuild): `provision.Provisioner` (enrol), `provision.Lifecycle` (suspend/resume/move/schedule-delete), `SettingsService`. **Platform** scope under `/admin` behind `RequireSession → RequirePlatformAdmin` with CSRF on mutations; NOT tenant-scoped (a platform admin acts across tenants; FR-ACC-03 governs `/v1`). `POST /admin/tenants` runs the provisioner synchronously (ADR-0016 precedent; async River `provision_tenant` *execution* is the one EPIC-09 seam) + records a `succeeded` `provision_tenant` mirror row (ADR-0005 history view, not a queued fake) → `{tenant, job_id}` 201. `GET /admin/tenants` (`?limit&cursor` → `{items,next_cursor}` keyset on `(created_at,id)`; no connection secrets, C-4). `PATCH /admin/tenants/{id}` fans out settings→`SettingsService.Patch` (FR-TEN-08), connection→`Lifecycle.Move` (FR-TEN-07, password re-encrypted C-4), status→`Suspend`/`Resume` (FR-TEN-04); unknown→404, illegal transition/immutable→409; no duplicate audit. `DELETE /admin/tenants/{id}?grace` (default 7 days) → `Lifecycle.ScheduleDelete` (FR-TEN-05: status→deleting, delete_after, 202). Wired into `internal/api` (`New` + `liveRoutes()`); `api/openapi.yaml` regenerated (drift guard green); no migration/schema change. `provision.ErrValidation`/`ErrIllegalTransition` exported (additive) for `errors.Is` status mapping. TDD; unit tests (`internal/cp/tenants`) + e2e golden path (`test/e2e/admin_tenants_e2e_test.go` — provisions a real tenant DB through the mounted router, CSRF-guard, suspend/settings/resume, schedule-delete, non-admin 403) over the real control-plane Postgres; `TestTenantIsolationSuite` re-run (SPEC-01 §9). Completes EPIC-04 (21/21). ADR-0032; ISSUE-0005.
