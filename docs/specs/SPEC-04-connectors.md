...
- HTML → markdown via readability-style extraction; `include_selectors`/`exclude_selectors` (CSS) prune the DOM, falling back to the semantic content-root heuristic when no include selectors are configured; title from `<title>`/`og:title`/`h1`. (Realised in §2b: the markdown render reuses the platform's `parse.htmlParser` engine, ADR-0034/0045 — not a separate `html-to-markdown` library.)
...
`internal/connector/webcrawl` extracts clean markdown for HTML pages behind the
STORY-07.1 parse seam (ADR-0045, ISSUE-0020, FR-SRC-05). An HTML page is now
emitted as `Document.Text` (markdown, `MimeType` `text/markdown`) rather than the
raw `Body`; non-HTML responses within the allowlist still pass through as the raw
`Body` for the parse sidecar (unchanged).

- **Selectors (`content.go`).** `include_selectors`/`exclude_selectors` are CSS
  selectors compiled with `github.com/andybalholm/cascadia` (the single new
  dependency; it needs only the already-vendored `golang.org/x/net/html`).
  `exclude_selectors` always drop their matching subtrees; `include_selectors`,
  when configured, keep ONLY their top-most matches (nested matches are not
  rendered twice).
- **Readability fallback.** With no usable include match, the whole
  (exclude-applied) document is handed to the platform's existing semantic
  extractor — `parse.htmlParser` (ADR-0034): it picks the `<main>`/`<article>`/
  `<body>` content root, skips known chrome (nav/aside, body-level header/footer,
  script/style/form + a class/id/role token regex covering cookie/consent/advert/
  share/related/comment/newsletter/…), and renders the result to markdown
  (headings, lists, GFM tables, code, inline links preserved).
- **Why not `go-readability`/`html-to-markdown`.** ADR-0034 already decided the
  platform's HTML boilerplate removal, deliberately rejecting a maintained
  readability port (forces a Go-toolchain bump past the repo's `go 1.22` pin — the
  actively-maintained `go-shiori/go-readability` HEAD requires go 1.23 and is
  deprecated) and an html-to-markdown library (collapses the heading/table
  structure). STORY-07.3 honours that decision and reuses the engine; it adds only
  the per-source CSS-selector capability (ADR-0045).
- **Title precedence.** `<title>` → `og:title` → first `<h1>` (`extract.go`),
  independent of the content extraction.
- **Acceptance.** A 20-page synthetic-but-representative golden corpus
  (`testdata/corpus/`, generated by construction) with committed markdown
  baselines and a reproducible boilerplate-removal metric (mean fraction of known
